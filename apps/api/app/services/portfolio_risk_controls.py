...
def stress_test(
    *,
    ...
    unable = set(suspended_codes or set()) | set(
        consecutive_limit_down_codes or set()
    )
    ...
    for code, value, adv in zip(
        codes, position_values, adv_amounts, strict=True
    ):
        days_normal = (
            abs(float(value)) / max(float(adv) * normal_participation, 1.0)
        )
        days_stress = (
            None
            if code in unable
            else abs(float(value))
            / max(float(adv) * 0.30 * normal_participation, 1.0)
        )
        liquidation[code] = {
            ...
            "reason": (
                "suspended_or_consecutive_limit_down"
                if code in unable
                else None
            ),
        }
    ...
    return {
        ...
        "cash_requirement": sum(
            max(-float(row["portfolio_pnl_rate"]), 0.0)
            for row in reports
        ),
        "passed": (
            float(worst["portfolio_pnl_rate"]) >= -0.30 and not unable
        ),
...
def discretize_portfolio(
    *,
    ...
    for code in codes:
        ...
        shares[code] = int(
            math.floor(
                portfolio_value * float(target_weights[code]) / price / lot
            )
            * lot
        )
    actual_values = {
        code: shares[code] * float(prices[code]) for code in codes
    }
    ...
    if covariance is not None and benchmark_weights is not None:
        active = np.asarray(
            [
                actual_weights.get(code, 0.0)
                - float(benchmark_weights.get(code, 0.0))
                for code in codes
            ]
        )
    ...
    return {
        ...
        "weight_deviation": {
            code: actual_weights[code] - float(target_weights[code])
            for code in codes
        },
...
def capacity_curve(
    *,
    ...
    for capital in sorted(float(value) for value in capital_levels):
        ...
        approved = (
            net_return > 0
            and stress_days <= approved_max_liquidation_days
        )
        if approved:
