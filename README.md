...
当前活动版本 16 只属于运行链路影子验证；它复用版本 11 的冻结权重，
没有读取新的正式留出集，也不具备投资批准资格。主动收益与统计门禁仍未通过，
既不能解释为策略有效，也未完成真实资金生产演练。第二轮边界和逐项证据见
...
当前可信运行影子为版本 16，绑定代码提交 `78564df` 和验证哈希
`e7bbe64790f5887fd57784f4d2f55d94652b4880e2fb3a9c4d32d72adf909c6d`。
空账户 8 的观察期为 2026-08-06～2026-10-06；首日只生成信号，连续优化的
26 只目标经 A 股整手离散后形成 23 只可执行目标，股票仓位 90.9591%，其余
持有现金，需等下一真实交易日开盘模拟执行。版本 13 因运行代码与冻结提交
不一致、版本 14 因连续权重未离散即进入前向信号均已退役，相关运行只保留作
事故审计；版本 15 因缺少每次推进前的源码冻结门禁被版本 16 替代。详细证据见
[`docs/strategy-v16-operational-shadow-20260806.md`](docs/strategy-v16-operational-shadow-20260806.md)。
该两个月批次只评价数据、信号、成交、账本和对账，不是收益保证或投资验证。
...
深度主源、东方财富为历史回退；收盘快照优先东方财富、失败回退新浪并以
腾讯批量行情作为收盘兜底，所有回退结果必须校验交易日期和 OHLCV 合法性；
